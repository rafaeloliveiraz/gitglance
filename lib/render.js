...
function styleTokens(style, t) {
  switch (style) {
    ...
    case "gradient":
      return { radius: 18, font: SANS, mono: SANS, labelUpper: false, labelSpacing: 0.3, dots: false, divider: false, watermark: null, gradientBg: true };
    case "outline":
...
function background(W, H, t, tok, opts) {
  ...
  if (tok.gradientBg) {
    return glow + `<defs><linearGradient id="gg-grad" x1="0" y1="0" x2="1" y2="1">
        <stop offset="0%" stop-color="#${t.bg}"/>
        <stop offset="55%" stop-color="#${t.bg}"/>
        <stop offset="100%" stop-color="#${t.accent}" stop-opacity="0.35"/>
      </linearGradient></defs>
      <rect width="${W}" height="${H}" rx="${radius}" fill="url(#gg-grad)"/>` + border;
  }
  if (tok.glass) {
    return `
      <defs>
...
function renderDonut(data, opts) {
  ...
  rows.forEach((r, i) => {
    ...
    const color = r.name === "Other" ? t.muted : (LANG_COLORS[r.name] || t.accent);
    segs += `<path d="${arcPath(cx, cy, R, angle + 1.1, angle + 1.1 + sweep)}" stroke="#${color}" stroke-width="${STROKE}" fill="none" stroke-linecap="butt"${glowAttr(tok)}/>`;
    angle += (r.pct / 100) * 360;
  });
  // center label
...
function renderBars(data, opts) {
  ...
  rows.forEach((r, i) => {
    ...
    bars += `<rect class="bar-fill" x="${x.toFixed(1)}" y="${(baseY - bh).toFixed(1)}" width="${bw.toFixed(1)}" height="${bh.toFixed(1)}" rx="5" fill="#${color}"${glowAttr(tok)}/>
