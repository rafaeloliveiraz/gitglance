...
function background(W, H, t, tok, opts) {
  ...
  if (tok.bgKind === "gradient") {
    // Rich diagonal gradient with a soft radial highlight.
    return glow + `<defs>${clip}
      <linearGradient id="gg-g1" x1="0" y1="0" x2="1" y2="1">
        ...
        <stop offset="100%" stop-color="#${t.title}" stop-opacity="0.28"/>
      </linearGradient>
      <radialGradient id="gg-hl" cx="0.15" cy="0.0" r="0.9">
        <stop offset="0%" stop-color="#ffffff" stop-opacity="0.10"/>
        <stop offset="45%" stop-color="#ffffff" stop-opacity="0"/>
      </radialGradient></defs>
      <rect width="${W}" height="${H}" rx="${radius}" fill="#${t.bg}"/>
      <rect width="${W}" height="${H}" rx="${radius}" fill="url(#gg-g1)"/>
      <rect width="${W}" height="${H}" rx="${radius}" fill="url(#gg-hl)"/>` + border;
  }
  if (tok.bgKind === "aurora") {
    ...
    return glow + `<defs>${clip}
      ...
      <style>
        .gg-blob1 { animation: gg-drift1 14s ease-in-out infinite alternate; }
        .gg-blob2 { animation: gg-drift2 18s ease-in-out infinite alternate; }
        @keyframes gg-drift1 { from { transform: translate(0,0); } to { transform: translate(${Math.round(W * 0.08)}px, 14px); } }
        @keyframes gg-drift2 { from { transform: translate(0,0); } to { transform: translate(-${Math.round(W * 0.06)}px, -12px); } }
      </style></defs>
      <rect width="${W}" height="${H}" rx="${radius}" fill="#${t.bg}"/>
...
function animStyle() {
  ...
  return `<style>
    ...
    .vbar { transform-origin: center bottom; animation: gg-rise 0.9s ease-out; }
    .gg-seg { animation: gg-fade 0.8s ease-out; }
    .gg-ring { animation: gg-fade 1s ease-out; }
...
function renderRings(data, opts) {
  ...
  const title = opts.custom_title || `${data.name}'s GitHub Stats`;
  const stats = [
    ["Stars", data.stars, t.accent],
    ["Repos", data.public_repos, LANG_COLORS.TypeScript],
    ["Followers", data.followers, LANG_COLORS.JavaScript],
    ["Following", data.following, LANG_COLORS.Vue || "41b883"],
  ];
...
function renderBars(data, opts) {
  ...
  rows.forEach((r, i) => {
    ...
    const color = LANG_COLORS[r.name] || t.accent;
