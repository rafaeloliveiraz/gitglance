...
function titleBlock(title, t, tok, opts, x, y, W) {
  ...
  const bar = tok.titleBar && opts.theme !== "kangel" ? `<rect x="${x}" y="${y + 10}" width="46" height="3" rx="1.5" fill="#${color}"/>` : "";
...
function renderRings(data, opts) {
  ...
  const ringCols = tok.monochrome
    ...
    : opts.theme === "kangel"
      ? ["4d21cb", "6cb2e0", "e64c55", "8cf0e0"]
      : [t.accent, LANG_COLORS.TypeScript, LANG_COLORS.JavaScript, LANG_COLORS.Vue || "41b883"];
  const ringTrack = !tok.monochrome && opts.theme === "kangel" ? "ee98ee" : t.border;
  const stats = [
