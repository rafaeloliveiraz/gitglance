...
function background(W, H, t, tok, opts) {
  ...
  if (tok.bgKind === "vlines") {
    ...
    for (let ly = 26; ly < H; ly += 26) {
      lines += `<line x1="0" y1="${ly}" x2="${W}" y2="${ly}" stroke="#${t.text}" stroke-opacity="0.05" stroke-width="1"/>`;
    }
    return glow + `<defs>${clip}
  ...
  if (tok.bgKind === "grid") {
    // Grid pattern with zones that fade in and out at different rhythms.
    const zw = Math.ceil(W / 3);
    return glow + `<defs>${clip}
      <pattern id="gg-gridp" width="26" height="26" patternUnits="userSpaceOnUse">
        <path d="M26 0H0V26" fill="none" stroke="#${t.text}" stroke-opacity="0.14" stroke-width="1"/>
      </pattern>
      <style>
        .gg-z1 { animation: gg-zone 3.2s ease-in-out infinite alternate; }
        .gg-z2 { animation: gg-zone 4.1s ease-in-out 0.8s infinite alternate; }
        .gg-z3 { animation: gg-zone 5s ease-in-out 1.6s infinite alternate; }
        @keyframes gg-zone { from { opacity: 0.15; } to { opacity: 1; } }
      </style></defs>
      <rect width="${W}" height="${H}" rx="${radius}" fill="#${t.bg}"/>
      <g clip-path="url(#gg-clip)">
        <rect class="gg-z1" x="0" width="${zw}" height="${H}" fill="url(#gg-gridp)"/>
        <rect class="gg-z2" x="${zw}" width="${zw}" height="${H}" fill="url(#gg-gridp)"/>
        <rect class="gg-z3" x="${zw * 2}" width="${zw}" height="${H}" fill="url(#gg-gridp)"/>
      </g>` + border;
  }
  if (tok.bgKind === "orbs") {
